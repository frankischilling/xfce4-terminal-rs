...
# only this machine knows. The second candidate names this host and carries a path
# that stops the conversion, which is what separates a host that was read and
# matched from a host that was never read at all.
fixtures=$test_root/fixtures.txt
cp "$script_dir/link-fixtures.txt" "$fixtures"
printf 'file://%s/tmp/example\n' "$(uname -n)" >> "$fixtures"
printf 'file://%s/tmp/%%zz\n' "$(uname -n)" >> "$fixtures"
...
# Both probes decide for themselves which lines of the corpus are candidates, so
...
candidates=$(grep -cv '^#' "$fixtures")
reported=$(grep -c '^classify	' "$test_root/reference.tsv")
if [ "$reported" -ne "$candidates" ]; then
  echo "$0: reported $reported of $candidates candidates" >&2
  exit 1
fi
...
grep -q "^clickable	file://$(uname -n)/tmp/%zz	true\$" "$test_root/reference.tsv"
...
# The table order decides these, not the scheme they name.
grep -q '^classify	file:///tmp/www\.example\.com	http$' "$test_root/reference.tsv"
grep -q '^classify	news:comp\.lang\.c@example\.com	email$' "$test_root/reference.tsv"
