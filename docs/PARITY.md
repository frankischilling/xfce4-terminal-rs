...
| Terminal screen | PTY environment, child lifecycle, titles, colors, search, paste, scrolling, and the highlighting the same patterns drive inside VTE | Not started |
...
reach. A probe may read one when the behavior under test cannot be entered from
outside, provided the values it reports still come from frozen code. The link
probe reads the widget's compiled patterns for that reason: the function that
uses them takes a `GdkEvent` and needs a live terminal, so the classification
cannot be requested directly, but the patterns, their compile options, and their
order remain the reference's own.
